...
aud_raw=open(AUDF,encoding="utf-8").read() if os.path.exists(AUDF) else ""
...
aud_body,_=strip_fm(aud_raw) if aud_raw else ("","")
...
# 読者別（営業・技術・製造）
AUD={
 "S":{"key":"sales","title":"営業向け","emoji":"🤝","who":"営業","word":"品質は“約束”。約束できないことは持ち帰る"},
 "T":{"key":"tech","title":"技術向け","emoji":"🛠️","who":"生産技術・開発","word":"前提が変わる＝新しいリスクが生まれる"},
 "M":{"key":"mfg","title":"製造向け","emoji":"🏭","who":"製造現場","word":"異常はまず止める、それから直す"},
}
...
<footer class="hb-foot"><div class="wrap">
  ...
  <div class="hb-foot-sub">本文の正（Single Source of Truth）は <a href="content/playbooks.md">場面別プレイブック集.md</a> ／ <a href="content/policy-master.md">方針マスター.md</a> ／ <a href="content/audience-guides.md">読者別ガイド.md</a>。最終更新 {LAST_UPDATED}。運用は <a href="content/README.md">README</a> 参照。</div>
</div></footer>
...
rev_aud=collect_review(aud_body) if aud_body else []
...
{rev_block("読者別ガイド 由来","content/audience-guides.md",rev_aud) if rev_aud else ""}'''
...
search_index.append({"url":"review.html","title":"付録：要確認リスト","g":"要確認","t":"要確認 未確定 "+ " ".join(rev_master+rev_play+rev_aud)})
...
def parse_aud(body):
    ...
    for raw in body.split("\n"):
        line=raw.rstrip()
        mg=re.match(r"^#\s+([STM])\.\s+(.*)$", line)
        mt=re.match(r"^##\s+([STM][0-9]+)\.\s+(.*)$", line)
        if mg:
...
if agroups:
    ...
    for t in atopics:
        ...
        if purpose:
            ...
            lab=plabel.group(1) if plabel else "目的"
            pbox=f'<div class="purpose"><span class="p-tag">{esc(lab)}</span><p>{inline(ptxt)}</p></div>'
        # related scenarios (cross-link to playbooks A-D)
        relhtml=""
        rels=[r for r in t["related"] if r in sc_title]
        if rels:
            links="".join(f'<a href="{slug(r)}"><span class="rel-id">{esc(r)}</span>{esc(sc_title[r])}</a>' for r in rels)
            relhtml=f'<div class="rel"><h3>関連する場面（品証プレイブック）</h3><div class="rel-list">{links}</div></div>'
        # siblings in same audience
    ...
    for code,ai in AUD.items():
        ...
        cards="".join(
            f'<a class="sc-card" href="{aslug(x["id"])}"><span class="sc-id">{esc(x["id"])}</span><span class="sc-t">{esc(x["title"])}</span></a>'
            for x in tps)
        body=f'''<h1 class="g-title"><span class="g-emoji">{ai["emoji"]}</span>{esc(ai["title"])}</h1>
    <p class="g-sub">連携先：{esc(ai["who"])}　|　{len(tps)} トピック</p>
    {('<div class="g-note">'+note_html+'</div>') if note_html else ''}
    <div class="sc-grid">{cards}</div>'''
        bc=crumb(("ハンドブック","index.html"),("読者別","aud-index.html"),(ai["title"],None))
        open(os.path.join(OUT,ahub(code)),"w",encoding="utf-8").write(
            page(ai["title"], bc, body, "aud-index.html", desc=f"{ai['title']}：品証との連携ガイド"))
    ...
    for code,ai in AUD.items():
        ...
        gc+=f'''<div class="hub-gcard g-{ai["key"]}">
    ...
    aud_hub_cards=gc
    intro=render_blocks([l for l in aud_body.split("\n")][:0])  # placeholder
    body=f'''<div class="hub-hero">
  <h1>読者別ハンドブック（連携ガイド）</h1>
  <p>品質保証グループ（QA）から、営業・技術・製造の各部門へ向けた連携の手引きです。「品証と、いつ・何を・どう連携するか」を場面別にまとめています。各トピックの末尾から、品証内プレイブック（A〜D）へ辿れます。</p>
</div>
<section class="hub-sec"><h2>読者別に探す</h2><div class="hub-gcards">{gc}</div></section>
<section class="hub-sec links"><h2>関連</h2>
  <a href="policy.html">📘 方針マスター（6原則・エスカレーション基準）</a>
  <a href="index.html">🏠 ハンドブック トップ</a>
</section>'''
    bc=crumb(("ハンドブック","index.html"),("読者別",None))
    open(os.path.join(OUT,"aud-index.html"),"w",encoding="utf-8").write(
        page("読者別ハンドブック", bc, body, "aud-index.html", desc="営業・技術・製造 向けの品証連携ガイド"))
    search_index.append({"url":"aud-index.html","title":"読者別ハンドブック（営業・技術・製造）","g":"読者別","t":"読者別 営業 技術 製造 連携 "+plain_text(aud_body.split("\n"))})
...
aud_sec=""
if aud_hub_cards:
    aud_sec=f'''<section class="hub-sec"><h2>読者別ガイド（営業・技術・製造）</h2>
  <p class="g-sub">品証から各部門への連携の手引き。「いつ・何を・どう連携するか」を場面別に。</p>
  <div class="hub-gcards">{aud_hub_cards}</div></section>'''
body=f'''<div class="hub-hero">
...
<section class="hub-sec links"><h2>その他</h2>
  <a href="aud-index.html">👥 読者別ガイド（営業・技術・製造）</a>
...
open(os.path.join(OUT,"index.html"),"w",encoding="utf-8").write(
    page("ハンドブック","<span>ハンドブック</span>",body,"index.html",
         desc="品質保証グループ 場面別ハンドブック（検索・4グループ・読者別・25場面）"))
...
print("audiences:",{c:len(agroups[c]["topics"]) for c in agroups} if agroups else "none")
print("principles:",len(principles))
print("review items: master",len(rev_master),"play",len(rev_play),"aud",len(rev_aud))
